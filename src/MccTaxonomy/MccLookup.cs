...
public static class MccLookup
{
    ...
    /// <summary>
    /// Tries to get the category for a given MCC string.
    /// Returns <c>false</c> if the string is <c>null</c>, malformed, or not present.
    /// </summary>
    public static bool TryGetCategory(string? mccCode, out MccCategory category)
        => _default.TryGetCategory(mccCode, out category);

    /// <summary>
    /// Tries to get the category for a given MCC character span. Allocation-free.
    /// Returns <c>false</c> if the span is malformed or not present.
    /// </summary>
    public static bool TryGetCategory(ReadOnlySpan<char> mccCode, out MccCategory category)
        => _default.TryGetCategory(mccCode, out category);
    ...
    /// Setting a code to <see cref="MccCategory.Uncategorized"/> removes it.
    /// </param>
    ...
    private static MccLookupInstance BuildDefault()
    {
        ...
        Set(t, occ, MccCategory.Marketing,
            7311);

        SetRange(t, occ, MccCategory.Airlines, 3000, 3350);
        Set(t, occ, MccCategory.Airlines,
            4415, 4511);

        Set(t, occ, MccCategory.Automotive,
            4784, 5013, 5172, 5271,
        ...
        SetRange(t, occ, MccCategory.VehicleRental, 3351, 3439);
        Set(t, occ, MccCategory.VehicleRental,
            3441, 7512, 7513, 7519);

        Set(t, occ, MccCategory.Charity,
            8398, 8641, 8651, 8661, 8675, 8699, 8734, 8743);

        Set(t, occ, MccCategory.Construction,
        ...
        SetRange(t, occ, MccCategory.FoodAndDining, 5811, 5814);

        Set(t, occ, MccCategory.DirectSales,
            4761, 5964, 5965, 5966, 5967, 5969);

        Set(t, occ, MccCategory.Education,
            8211, 8220, 8241, 8244, 8249, 8299);

        Set(t, occ, MccCategory.Electronics,
            5045, 5722, 5732, 5946);

        Set(t, occ, MccCategory.Leisure,
            5733, 5735, 5971, 7032, 7829, 7832, 7833, 7841,
            7911, 7922, 7929, 7932, 7933, 7941);
        SetRange(t, occ, MccCategory.Leisure, 7991, 7994);
        SetRange(t, occ, MccCategory.Leisure, 7996, 7999);

        Set(t, occ, MccCategory.Finance,
            6010, 6011, 6012, 6050, 6051, 6211, 6529, 6530,
            6533, 6535, 6536, 6537, 6538, 6539, 6540, 6611,
            6760, 7276, 7321, 7322, 8931, 9700, 9701, 9702);

        Set(t, occ, MccCategory.Betting,
            7800, 7801, 7802, 7995, 9406, 9754);

        Set(t, occ, MccCategory.Government,
            9211, 9222, 9223, 9311, 9312, 9313, 9314, 9399,
            9401, 9402, 9405, 9034);

        Set(t, occ, MccCategory.Supermarkets,
            5300, 5411, 5412, 5422, 5441, 5451, 5462, 5499,
            9751);

        Set(t, occ, MccCategory.Healthcare,
            5047, 5122, 5292, 5912, 5975, 5976, 5977, 7230,
            7297, 7298, 8011, 8021, 8031, 8041, 8042, 8043,
            8044, 8049, 8050, 8062, 8071, 8099);

        SetRange(t, occ, MccCategory.Accommodation, 3501, 3839);
        Set(t, occ, MccCategory.Accommodation,
            7011, 7012, 7033);

        Set(t, occ, MccCategory.Insurance,
            5960, 6300, 6381, 6399);

        Set(t, occ, MccCategory.DigitalServices,
            4816, 5262, 5734, 5815, 5816, 5817, 5818, 5968,
            7372, 7375, 7379, 7399);

        Set(t, occ, MccCategory.Retail,
        ...
        Set(t, occ, MccCategory.GeneralServices,
            742,  763,  780,
        ...
        Set(t, occ, MccCategory.OfficeSupplies,
            5943);

        Set(t, occ, MccCategory.Transportation,
            4011, 4111, 4112, 4119, 4121, 4131, 4411, 4457,
            4468, 4582, 4789, 5551, 6236);

        Set(t, occ, MccCategory.TravelAgencies,
            4722, 4723);

        Set(t, occ, MccCategory.Utilities,
            4812, 4813, 4814, 4821, 4899, 4900);

        return new MccLookupInstance(t, occ);
    }

    private static void Set(MccCategory[] t, bool[] occ, MccCategory cat, params int[] codes)
    {
    ...
    private static void SetRange(MccCategory[] t, bool[] occ, MccCategory cat, int from, int to)
    {
